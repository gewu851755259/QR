...
public class PlayAcy extends Activity implements OnCompletionListener,OnClickListener {
	private String path;
	private Uri uri;
	...
	private void setAnimations()
    {
	 ...
	 private class SingleGestureListener extends android.view.GestureDetector.SimpleOnGestureListener {
			...
			public boolean onScroll(MotionEvent e1, MotionEvent e2,
				...
				float moldY = e1.getY();
				float y = e2.getY();
				if (moldX > screenBean.getsWidth() * 4.0 / 5)// 在屏幕的4/5处上下滑动
					changeVolume((moldY - y) / screenBean.getsHeight());
				else if (moldX < screenBean.getsWidth() / 5.0)// 在屏幕1/5处上下滑动
...
//	显示电池电量和时间
	private void getTime(){
 	...
 	Thread thread = new delayThread(300);
 	thread.start();
 }
